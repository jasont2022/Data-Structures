...
/**
 * Implements the Heap Interface using a Binary Max Heap with ArrayList and HashMap for faster
 * operations (in terms of Runtime), this implementation does not support null elements
 *
 * @author Jason Tran
 */
...
=======
  ...
  /**
   * Constructor: Creates a BinaryMaxHeap with the array, similar to the Build Heap Algorithm,
   * Runtime: O(n)
   *
   * @param array an array to be constructed into a heap
   */
  public BinaryMaxHeap(E[] array) {
    ...
    for (int i = 0; i < size; i++) {
      mapAdd(array[i], i);
      arr.add(array[i]);
>>>>>>> 14a7b816714d3ee54ca2bdbab52aef0e2d58a60c
    }
  ...
  /**
   * This returns the underlying array that stores the collection of elements, Runtime: O(1)
   *
   * @return the underlying array
   */
  public List<E> getArray() {
...
>>>>>>> 14a7b816714d3ee54ca2bdbab52aef0e2d58a60c
  ...
  /**
   * Helper method that puts a key-value pair into the HashMap, Runtime: Expected O(1)
   *
   * @param e an element to be added to the map
   * @param i the index to be added to the set of indices
   */
